...
if uploaded_file is not None:
    ...
    if probs.max().item() > 0.7:
        out = st.write(f"I am guessing this is a {label[0]} with a probability of: {probs.max().item() * 100:.04f}%")
    else:
        out = st.write("Excuse me I am not sure what this is")
